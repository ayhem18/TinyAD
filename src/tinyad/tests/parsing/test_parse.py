...
from tinyad.parsing.parser import evaluate_expression, evaluate_expression_inner, extend_expression, parse_no_operators2, parse_number, parse_variable
...
class TestParserLevel2Expressions(ParserBaseTest):
    ...
    def test_evaluate_expression(self):
        for _ in range(20000):
            ...
            extended_expression, variables, new_precomputed = extend_expression(final_implicit_expression, precomputed_indices, {})
            
            self.assertEqual(extended_expression, final_explicit_expression)

            evaluated_var = evaluate_expression_inner(extended_expression, variables, new_precomputed)

            self.assertEqual(evaluated_var.name, final_explicit_expression) 
